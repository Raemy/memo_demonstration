/* ===== PLATEFORME EXPÉRIMENTALE v5 ===== */
/* Architecture mono-poste : 1 seul ordinateur, expérimentateur + participant */
...
const TACHES_ENTRAINEMENT = [
  { id:'E1', label:'Trouver le numéro d\'assurance maladie', enonce:'Trouvez votre numéro d\'assurance maladie. Il se trouve dans le bas de la page.', reponse:'Numéro visible dans le footer.' },
  { id:'E2', label:'Se déconnecter', enonce:'Déconnectez-vous de l\'application.', reponse:'Bouton "Me déconnecter" / "Se déconnecter" en haut à droite.' },
  { id:'E3', label:'Lire les CGU', enonce:'Lisez les Conditions Générales d\'Utilisation (CGU).', reponse:'Lien CGU dans le footer.' }
];
...
const NASA_DIMENSIONS = [
  { id:'mentale',     label:'Exigence Mentale',    desc:'Etait-ce difficile à comprendre et à utiliser ?' },
  ...
  { id:'temporelle',  label:'Exigence Temporelle', desc:'Avez-vous eu l\'impression de devoir aller vite ?' },
  { id:'performance', label:'Performance',         desc:'Dans quelle mesure pensez-vous avoir réussi ce qui vous était demandé ?' },
  { id:'effort',      label:'Effort',              desc:'Avez-vous dû faire beaucoup d\'efforts' },
  { id:'frustration', label:'Frustration',         desc:'Dans quelle mesure vous êtes-vous senti·e irrité·e, stressé·e ou frustré·e ?' }
];

// ─── ÉTAT GLOBAL ───
const State = {
  participant: { code:'', groupe:'', typeGroupe:'', questionnaireMode:'', age:'', genre:'', niveauEtudes:'', aisanceNumerique:'' },
  sequence: [], plan: [], tacheIdx: 0,
  donnees: [],
  questionnaires: { sus:{}, nasaTlxInterface:{}, nasaTlxTache:{} },
...
const Chrono = {
  start() {
    ...
    State.chrono.interval = setInterval(() => {
      State.chrono.elapsed = performance.now() - State.chrono.startTime;
      Chrono.render();
    }, 50);
  ...
  render() {
    const el = document.getElementById('chrono-display');
    if (el) el.textContent = Chrono.format(State.chrono.elapsed);
  },
  format(ms) {
...
// ─── CLICS (comptage local — expérimentateur clique manuellement +1 ou l'iframe reporte) ───
const Clics = {
  reset() { State.clics.count = 0; Clics.render(); },
  increment() { State.clics.count++; Clics.render(); },
...
const Data = {
  enregistrerTache({ tache, tempsDecompose, clics, reussite, note }) {
    ...
    State.donnees.push({
      ...
      genre:              State.participant.genre,
      niveau_etudes:      State.participant.niveauEtudes,
      aisance_numerique:  State.participant.aisanceNumerique,
  ...
  save() {
    try {
      localStorage.setItem('exp_v5_donnees',       JSON.stringify(State.donnees));
      localStorage.setItem('exp_v5_participant',    JSON.stringify(State.participant));
      localStorage.setItem('exp_v5_questionnaires', JSON.stringify(State.questionnaires));
      localStorage.setItem('exp_v5_sequence',       JSON.stringify({ sequence:State.sequence, plan:State.plan, tacheIdx:State.tacheIdx }));
    } catch(e) {}
  },
  load() {
    try {
      const d = localStorage.getItem('exp_v5_donnees');       if (d) State.donnees = JSON.parse(d);
      const p = localStorage.getItem('exp_v5_participant');    if (p) Object.assign(State.participant, JSON.parse(p));
      const q = localStorage.getItem('exp_v5_questionnaires'); if (q) Object.assign(State.questionnaires, JSON.parse(q));
      const s = localStorage.getItem('exp_v5_sequence');
      if (s) { const parsed = JSON.parse(s); State.sequence=parsed.sequence||[]; State.plan=parsed.plan||[]; State.tacheIdx=parsed.tacheIdx||0; }
    } catch(e) {}
  },
  resetQuestionnaires() {
    // Conserve le pré-questionnaire déjà rempli
    const pre = State.questionnaires.preFiltrage;
    const preAis = State.questionnaires.aisanceNumerique;
    State.questionnaires = { sus:{}, nasaTlxInterface:{}, nasaTlxTache:{}, preFiltrage: pre||{}, aisanceNumerique: preAis||{} };
    localStorage.setItem('exp_v5_questionnaires', JSON.stringify(State.questionnaires));
  },
  resetAllQuestionnaires() {
    State.questionnaires = { sus:{}, nasaTlxInterface:{}, nasaTlxTache:{}, preFiltrage:{}, aisanceNumerique:{} };
    localStorage.setItem('exp_v5_questionnaires', JSON.stringify(State.questionnaires));
  },
  resetSession() {
    State.tacheIdx = 0; State.sequence = []; State.plan = [];
    localStorage.removeItem('exp_v5_sequence');
  ...
  exportTaches(filtres) {
    ...
    const hdrs = [
      'code_participant','date','heure','groupe_participant','groupe_latin','questionnaire_mode',
      'age','genre','niveau_etudes','aisance_numerique',
  ...
  exportQuestionnaires(filtres) {
    ...
    // Pré-questionnaire filtrage
    if (q.preFiltrage && Object.keys(q.preFiltrage).length) {
      const pf = q.preFiltrage;
      rows.push({ ...meta, instrument:'Pre-Filtrage',
        PRE_age: pf.age||'', PRE_niveau_francais: pf.niveau_francais||'',
        PRE_aisance_tactile: pf.aisance_tactile||'', PRE_genre: pf.genre||'',
        PRE_niveau_etude: pf.niveau_etude||'', PRE_discipline: pf.discipline||'',
        PRE_informatique: pf.informatique||'' });
    }

    // Pré-questionnaire aisance numérique
    if (q.aisanceNumerique && Object.keys(q.aisanceNumerique).length) {
      const an = q.aisanceNumerique;
      rows.push({ ...meta, instrument:'Pre-AisanceNumerique',
        PRE_annees_internet: an.annees_internet||'', PRE_support: an.support||'',
        PRE_frequence: an.frequence||'', PRE_temps_connexion: an.temps_connexion||'',
        PRE_temps_site: an.temps_site||'', PRE_competences: an.competences||'',
        PRE_difficultes: an.difficultes||'', PRE_difficultes_detail: an.difficultes_detail||'',
        PRE_comprehension_clic: an.comprehension_clic||'' });
    }

    Object.keys(q.sus||{}).forEach(ifaceId => {
      const sus = q.sus[ifaceId]; const iface = INTERFACES[ifaceId]||{};
      const row = { ...meta, instrument:'F-SUS', interface_id:ifaceId, interface_label:iface.shortLabel||'' };
    ...
    Object.keys(q.nasaTlxInterface||{}).forEach(ifaceId => {
      ...
      const row = { ...meta, instrument:'NASA-TLX-Interface', interface_id:ifaceId, interface_label:iface.shortLabel||'' };
      NASA_DIMENSIONS.forEach(dim => row[`NASA_I_${dim.id}`] = nasa[dim.id]??'');
      row['NASA_I_score_moyen'] = Data._scoreNASA(nasa);
      rows.push(row);
    });

    Object.keys(q.nasaTlxTache||{}).forEach(ifaceId => {
      const nasa = q.nasaTlxTache[ifaceId]; const iface = INTERFACES[ifaceId]||{};
      const row = { ...meta, instrument:'NASA-TLX-Taches', interface_id:iface.id||'', interface_label:iface.shortLabel||'' };
...
window.Utils=Utils; window.DateFR=DateFR;
